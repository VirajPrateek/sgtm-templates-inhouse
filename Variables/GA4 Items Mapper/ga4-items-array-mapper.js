...
switch (eventType) {
  ...
  case 'select_item':
    ...
    if (componentModuleCustName && componentModuleCustName !== 'na') {
      ...
      jsonData.prod_module_competition_config = (safeTransactionProducts['component.moduleCompetitionConfig'] || '').toLowerCase();
      jsonData.prod_module_affiliation_config = (safeTransactionProducts['component.moduleAffiliationConfig'] || '').toLowerCase();
      jsonData.prod_module_sports_config = (safeTransactionProducts['component.moduleSportsConfig'] || '').toLowerCase();
      jsonData.prod_module_source = (componentModuleSource || 'na').toLowerCase();
      jsonData.item_category_4 = (safeTransactionProducts['gameDetails'] || '').toLowerCase();
      jsonData.prod_league_id = (safeTransactionProducts['leagueID'] || '').toLowerCase();
      jsonData.prod_module_user_segment = (componentUserSegment || 'na').toLowerCase();
      jsonData.prod_recommendation_type = (safeTransactionProducts['component.recommendationtype'] || '').toLowerCase();
      jsonData.prod_marquee_content_logic = (safeTransactionProducts['marquee.contentLogic'] || '').toLowerCase();
    }
    else {
      ...
      jsonData.prod_module_name = (safeTransactionProducts['component.moduleName'] || '').toLowerCase();
      jsonData.prod_module_position = (safeTransactionProducts['component.modulePosition'] || '').toLowerCase();
      jsonData.prod_market_id = (safeTransactionProducts.marketID || 'na').toLowerCase();
      jsonData.prod_result_id = (safeTransactionProducts.resultID || 'na').toLowerCase();
      jsonData.prod_content_position = (safeTransactionProducts['component.ContentPosition'] || '').toLowerCase();
      jsonData.item_category3 = (safeTransactionProducts['component.pageLayout'] || '').toLowerCase();
      jsonData.prod_module_cust_name = (safeTransactionProducts['component.moduleCustName'] || '').toLowerCase();
      jsonData.prod_module_competition_config = (safeTransactionProducts['component.moduleCompetitionConfig'] || '').toLowerCase();
      jsonData.prod_module_affiliation_config = (safeTransactionProducts['component.moduleAffiliationConfig'] || '').toLowerCase();
      jsonData.prod_module_sports_config = (safeTransactionProducts['component.moduleSportsConfig'] || '').toLowerCase();
      jsonData.prod_module_source = (safeTransactionProducts['component.moduleSource'] || '').toLowerCase();
      jsonData.item_category_4 = (safeTransactionProducts['gameDetails'] || '').toLowerCase();
      jsonData.prod_league_id = (safeTransactionProducts['leagueID'] || '').toLowerCase();
